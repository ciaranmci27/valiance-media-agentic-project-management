...
BEGIN
  IF to_regclass('public.team_members') IS NULL
    ...
    OR to_regclass('public.project_time_entries') IS NULL
    OR to_regclass('public.api_keys') IS NULL THEN
    RAISE EXCEPTION 'Required application tables are missing';
  END IF;

  IF (SELECT COUNT(*) FROM public.team_members WHERE role = 'admin') <> 1
    AND NOT EXISTS (SELECT 1 FROM public.team_members WHERE role = 'owner') THEN
    RAISE EXCEPTION 'Expected exactly one existing admin to bootstrap as Owner';
  END IF;
END;
$$;
...
CREATE OR REPLACE TRIGGER set_role_permissions_updated_at
  BEFORE UPDATE ON public.role_permissions
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

CREATE OR REPLACE TRIGGER set_team_member_permissions_updated_at
...
CREATE TABLE IF NOT EXISTS public.team_member_hourly_rates (
...
CREATE TABLE IF NOT EXISTS public.team_member_earning_adjustments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS public.team_member_payout_allocations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_team_member_hourly_rates_lookup
  ON public.team_member_hourly_rates(member_id, effective_at DESC);
CREATE INDEX IF NOT EXISTS idx_team_member_earning_adjustments_member
  ON public.team_member_earning_adjustments(member_id, effective_date DESC);
CREATE INDEX IF NOT EXISTS idx_team_member_payouts_member
  ON public.team_member_payouts(member_id, payment_date DESC);
CREATE INDEX IF NOT EXISTS idx_team_member_payout_allocations_payout
  ON public.team_member_payout_allocations(payout_id);
CREATE INDEX IF NOT EXISTS idx_team_member_payout_allocations_time_entry
  ON public.team_member_payout_allocations(time_entry_id) WHERE time_entry_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_team_member_payout_allocations_adjustment
  ON public.team_member_payout_allocations(adjustment_id) WHERE adjustment_id IS NOT NULL;

CREATE OR REPLACE TRIGGER set_team_member_hourly_rates_updated_at
  BEFORE UPDATE ON public.team_member_hourly_rates
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_team_member_earning_adjustments_updated_at
  BEFORE UPDATE ON public.team_member_earning_adjustments
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE OR REPLACE TRIGGER set_team_member_payouts_updated_at
...
CREATE TABLE IF NOT EXISTS public.project_credential_members (
...
CREATE INDEX IF NOT EXISTS idx_project_credential_members_member
...
CREATE OR REPLACE TRIGGER protect_project_billing_fields
...
CREATE OR REPLACE TRIGGER apply_time_entry_compensation
...
BEGIN
  IF to_regclass('public.project_goals') IS NOT NULL THEN
    ...
    EXECUTE 'DROP POLICY IF EXISTS project_goals_access ON public.project_goals';
  ...
  IF to_regclass('public.task_suggestions') IS NOT NULL THEN
    ...
    EXECUTE 'DROP POLICY IF EXISTS task_suggestions_access ON public.task_suggestions';
  ...
  IF to_regclass('public.agent_activities') IS NOT NULL THEN
    ...
    EXECUTE 'DROP POLICY IF EXISTS agent_activities_access ON public.agent_activities';
  ...
  IF to_regclass('public.api_audit_log') IS NOT NULL THEN
    ...
    EXECUTE 'DROP POLICY IF EXISTS api_audit_log_access ON public.api_audit_log';
...
-- ===========================================================================
-- Legacy profile removal
-- ===========================================================================

-- Remove the legacy public.profiles table after proving that team_members
-- contains every authenticated identity and no external database object still
-- reads the old table. This migration intentionally does not use CASCADE.


-- Production may still have the original signup trigger function that wrote
-- to public.profiles. Replace it first so new users continue to be created in
-- team_members after the legacy table is removed.
...
DO $$
DECLARE
  unmatched_profiles integer;
  email_mismatches integer;
  dependency_list text;
BEGIN
  IF to_regclass('public.profiles') IS NULL THEN
    RAISE NOTICE 'public.profiles does not exist; nothing to remove';
    RETURN;
  END IF;

  IF to_regclass('public.team_members') IS NULL THEN
    RAISE EXCEPTION 'Cannot remove profiles because public.team_members does not exist';
  END IF;

  SELECT COUNT(*)
  INTO unmatched_profiles
  ...
  IF unmatched_profiles > 0 THEN
    RAISE EXCEPTION 'Cannot remove profiles: % profile rows are not represented in team_members', unmatched_profiles;
  END IF;

  SELECT COUNT(*)
  INTO email_mismatches
  FROM public.profiles profile
  JOIN public.team_members member ON member.auth_user_id = profile.id
  WHERE lower(COALESCE(profile.email, '')) IS DISTINCT FROM lower(COALESCE(member.email, ''));

  IF email_mismatches > 0 THEN
    RAISE EXCEPTION 'Cannot remove profiles: % matching identities have different email addresses', email_mismatches;
  END IF;

  SELECT string_agg(format('%I.%I (%I)', source_namespace.nspname, source_table.relname, constraint_row.conname), ', ')
  INTO dependency_list
  FROM pg_constraint constraint_row
  JOIN pg_class source_table ON source_table.oid = constraint_row.conrelid
  JOIN pg_namespace source_namespace ON source_namespace.oid = source_table.relnamespace
  WHERE constraint_row.confrelid = 'public.profiles'::regclass
    AND constraint_row.conrelid <> 'public.profiles'::regclass;

  IF dependency_list IS NOT NULL THEN
    RAISE EXCEPTION 'Cannot remove profiles because foreign keys still reference it: %', dependency_list;
  END IF;

  SELECT string_agg(format('%I.%I', view_row.schemaname, view_row.viewname), ', ')
  INTO dependency_list
  FROM pg_views view_row
  WHERE view_row.schemaname NOT IN ('pg_catalog', 'information_schema')
    AND view_row.definition ~* '(^|[^a-zA-Z0-9_])((public\.)?profiles)([^a-zA-Z0-9_]|$)';

  IF dependency_list IS NOT NULL THEN
    RAISE EXCEPTION 'Cannot remove profiles because views still reference it: %', dependency_list;
  END IF;

  SELECT string_agg(format('%I.%I', view_row.schemaname, view_row.matviewname), ', ')
  INTO dependency_list
  FROM pg_matviews view_row
  WHERE view_row.schemaname NOT IN ('pg_catalog', 'information_schema')
    AND view_row.definition ~* '(^|[^a-zA-Z0-9_])((public\.)?profiles)([^a-zA-Z0-9_]|$)';

  IF dependency_list IS NOT NULL THEN
    RAISE EXCEPTION 'Cannot remove profiles because materialized views still reference it: %', dependency_list;
  END IF;

  SELECT string_agg(format('%I.%I', function_namespace.nspname, function_row.proname), ', ')
  INTO dependency_list
  ...
  WHERE function_namespace.nspname NOT IN ('pg_catalog', 'information_schema')
    ...
    AND pg_get_functiondef(function_row.oid) ~* '(^|[^a-zA-Z0-9_])((public\.)?profiles)([^a-zA-Z0-9_]|$)';

  IF dependency_list IS NOT NULL THEN
    RAISE EXCEPTION 'Cannot remove profiles because functions still reference it: %', dependency_list;
  END IF;

  SELECT string_agg(format('%I.%I (%I)', policy_namespace.nspname, policy_table.relname, policy_row.polname), ', ')
  INTO dependency_list
  FROM pg_policy policy_row
  JOIN pg_class policy_table ON policy_table.oid = policy_row.polrelid
  ...
  WHERE policy_row.polrelid <> 'public.profiles'::regclass
    AND (
      COALESCE(pg_get_expr(policy_row.polqual, policy_row.polrelid), '')
        ~* '(^|[^a-zA-Z0-9_])((public\.)?profiles)([^a-zA-Z0-9_]|$)'
      OR COALESCE(pg_get_expr(policy_row.polwithcheck, policy_row.polrelid), '')
        ~* '(^|[^a-zA-Z0-9_])((public\.)?profiles)([^a-zA-Z0-9_]|$)'
    );

  IF dependency_list IS NOT NULL THEN
    RAISE EXCEPTION 'Cannot remove profiles because RLS policies still reference it: %', dependency_list;
  END IF;
END;
$$;
...
-- Agents propose work but do not receive the review permission that can approve
-- or reject suggestions. Agent activity logging is a separate API-only power.
...
-- Existing agent keys used the old combined suggestion scope. Preserve their
-- ability to propose work and log activity without retaining review authority.
...
BEGIN
  ...
  IF COALESCE(array_length(p_entry_ids, 1), 0) = 0 THEN
    RAISE EXCEPTION 'Select at least one time entry';
  END IF;
  IF p_decision NOT IN ('approved', 'rejected') THEN
    RAISE EXCEPTION 'Invalid review decision';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Active reviewer not found';
  END IF;
  IF (SELECT count(*) FROM public.project_time_entries WHERE id = ANY(p_entry_ids))
    <> cardinality(p_entry_ids) THEN
    RAISE EXCEPTION 'One or more time entries were not found';
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.project_time_entries
    WHERE id = ANY(p_entry_ids)
    ...
    RAISE EXCEPTION 'Time entry does not belong to the project';
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.project_time_entries
    WHERE id = ANY(p_entry_ids)
      AND member_id = p_reviewer_id
  ) THEN
    RAISE EXCEPTION 'Time entries cannot be self-approved';
  END IF;
...
BEGIN
  ...
  SET window_start = CASE
        WHEN api_rate_limit_windows.window_start + make_interval(secs => p_window_seconds) <= v_now
          THEN v_now ELSE api_rate_limit_windows.window_start END,
      request_count = CASE
        WHEN api_rate_limit_windows.window_start + make_interval(secs => p_window_seconds) <= v_now
