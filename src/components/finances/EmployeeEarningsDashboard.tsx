...
import type { EmployeeEarningsData, Project } from '@/lib/types';
...
export function EmployeeEarningsDashboard({
  projects,
  data,
}: {
  projects: Project[];
  ...
  const dashboard = useMemo(() => {
    ...
    const dayCount = Math.max(
      1,
      Math.round((today.getTime() - new Date(`${startKey}T00:00:00`).getTime()) / 86400000) + 1,
    );
    ...
    for (const entry of data.entries) {
      ...
      for (const [dateKey, workedHours] of getWorkedHoursByDay(entry)) {
        ...
        const projectRow = projectRows.get(entry.project_id) || {
          approved: 0,
          pending: 0,
          hours: 0,
        };
    ...
    for (const adjustment of data.adjustments) {
      if (
        ...
        adjustment.effective_date < startKey ||
        adjustment.effective_date > todayKey
      )
        continue;
      const amount =
    ...
    const paid = data.payouts
      .filter(
        (payout) =>
          !payout.voided_at && payout.payment_date >= startKey && payout.payment_date <= todayKey,
      )
    ...
    const { earned: approved, owed } = computeMemberEarningsSummary(data, {
      startKey,
      endKey: todayKey,
    });
  ...
  const visibleValues = dashboard.bars.map((bar) => {
    ...
    return {
      positive: Math.max(0, approved) + Math.max(0, pending),
      negative: Math.min(0, approved),
    };
  });
  ...
  const hasChartData = dashboard.bars.some(
    (bar) => Math.abs(bar.approved) > 0.005 || bar.pending > 0.005,
  );
      ...
      <section className="rounded-xl border border-white/[0.08] bg-surface-raised">
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-white">Daily Earnings</h2>
          </div>
        ...
        <div className="px-5 pb-4 pt-5">
          {!showApproved && !showPending ? (
            <ChartEmpty
              title="No data series selected"
              detail="Turn on Approved or Pending below"
            />
          ) : !hasChartData ? (
            <ChartEmpty
              title="No earnings in this range"
              detail="Submitted and approved time will appear here"
            />
          ) : (
            <div className="flex h-72 gap-2">
              <div className="flex w-12 flex-col justify-between py-0.5 text-right">
                {[...axisTicks].reverse().map((tick) => (
                  <span key={tick} className="text-[10px] font-medium text-zinc-500">
                    {formatAxis(tick)}
                  </span>
                ))}
              </div>
              <div className="relative flex-1">
                {axisTicks.map((tick) => (
                  <div
                    key={tick}
                    className={`absolute inset-x-0 border-t ${tick === 0 && axisMin < 0 ? 'border-white/[0.12]' : 'border-white/[0.06]'}`}
                ...
                <div className="relative z-[1] flex h-full items-stretch gap-[3px]">
                  {dashboard.bars.map((bar, index) => {
                    ...
                    const negativeHeight =
                      axisMin < 0 ? (Math.abs(approvedNegative) / Math.abs(axisMin)) * 100 : 0;
                    const labelStep = Math.max(1, Math.floor(dashboard.bars.length / 6));
                    const showLabel =
                      index === 0 || index === dashboard.bars.length - 1 || index % labelStep === 0;
                    return (
                      <div key={bar.dateKey} className="group relative min-w-0 flex-1">
                        <div
                          className="absolute inset-x-0 flex flex-col justify-end"
                          style={{ bottom: `${zeroBottom}%`, height: `${positiveArea}%` }}
                        >
                          {pending > 0 && (
                            <div
                              className="w-full rounded-t bg-amber-300 transition-colors group-hover:bg-amber-400"
                              style={{ height: `${Math.max(pendingHeight, 1.5)}%` }}
                            />
                          )}
                          {approvedPositive > 0 && (
                            <div
                              className={`w-full bg-emerald-500 transition-colors group-hover:bg-emerald-600 ${pending <= 0 ? 'rounded-t' : ''}`}
                              style={{ height: `${Math.max(approvedPositiveHeight, 1.5)}%` }}
                            />
                          )}
                        </div>
                        {approvedNegative < 0 && (
                          <div
                            ...
                            style={{ top: `${positiveArea}%`, height: `${negativeArea}%` }}
                          >
                            <div
                              className="w-full rounded-b bg-rose-400"
                              style={{ height: `${Math.max(negativeHeight, 2)}%` }}
                            />
                          </div>
                        )}
                        {(Math.abs(approved) > 0.005 || pending > 0.005) && (
                          <div className="pointer-events-none absolute bottom-[calc(100%+6px)] left-1/2 z-20 hidden -translate-x-1/2 whitespace-nowrap rounded-md bg-surface-overlay border border-white/[0.08] px-2.5 py-2 text-[10px] text-white shadow-lg group-hover:block">
                            ...
                            {Math.abs(approved) > 0.005 && (
                              <p>
                                Approved{' '}
                                <span className="ml-2 font-semibold">{formatMoney(approved)}</span>
                              </p>
                            )}
                            {pending > 0.005 && (
                              <p>
                                Pending{' '}
                                <span className="ml-2 font-semibold">{formatMoney(pending)}</span>
                              </p>
                            )}
                          </div>
                        )}
                        {showLabel && (
                          <span className="absolute top-[calc(100%+8px)] left-1/2 w-16 -translate-x-1/2 truncate text-center text-[10px] font-medium text-zinc-500">
                            {bar.label}
                          </span>
                        )}
                      </div>
      ...
      {dashboard.byProject.length > 0 && (
        <section className="overflow-hidden rounded-xl border border-white/[0.08] bg-surface-raised">
          ...
          <div className="grid grid-cols-[1fr_auto_auto_auto] gap-5 border-b border-white/[0.06] px-5 py-2 text-[10px] font-medium uppercase tracking-wider text-zinc-500">
            <span>Project</span>
            <span className="w-24 text-right">Approved</span>
          ...
          <div className="divide-y divide-white/[0.06]">
            {dashboard.byProject.map((project) => (
              <div
                ...
                className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-5 px-5 py-3.5"
              >
                ...
                <span className="w-24 text-right text-sm font-semibold text-emerald-400">
                  {formatMoney(project.approved)}
                </span>
                <span className="w-24 text-right text-sm font-semibold text-amber-400">
                  {formatMoney(project.pending)}
                </span>
                <span className="w-20 text-right text-sm font-medium text-violet-400">
...
function Stat({
  ...
  const color =
    tone === 'approved' ? 'text-emerald-400' : tone === 'pending' ? 'text-amber-400' : 'text-white';
  return (
    <div>
...
function ChartEmpty({ title, detail }: { title: string; detail: string }) {
  return (
    <div className="flex h-72 flex-col items-center justify-center text-center">
      <span className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-white/[0.06]">
...
function LegendButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`flex items-center gap-1.5 rounded-md px-2 py-1 transition-colors ${active ? activeClass : 'opacity-40 hover:opacity-70'}`}
    >
      <span className={`h-2.5 w-2.5 rounded-sm ${active ? color : 'bg-zinc-300'}`} />
