...
const BOX_BASE =
  'h-4 w-4 flex flex-shrink-0 items-center justify-center rounded-[5px] border transition-all duration-150';
// Explicit overrides: solid, bold ; they stand out from the inherited rows.
const boxCheck = (
  <span className={`${BOX_BASE} border-input-accent bg-input-accent text-input-accent-fg`}>
    <Check size={11} strokeWidth={3} />
  </span>
);
const boxDeny = (
  <span className={`${BOX_BASE} border-red-500 bg-red-500 text-white`}>
...
export function AccessControlModal({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMemberId((current) => {
        ...
        return current && members.some((member) => member.id === current)
          ? current
          : members[0]?.id || '';
      });
    } catch (error) {
  ...
  const applyRole = (current: AccessData, permission: PermissionKey, on: boolean): AccessData => ({
    ...current,
    role_permissions: [
      ...current.role_permissions.filter(
        (row) =>
          !(
            row.role === role &&
            row.permission_key === permission &&
            row.access_channel === channel
          ),
      ),
      ...(on
        ? [{ role, permission_key: permission, access_channel: channel } as RolePermission]
        : []),
    ],
  });
  const applyMember = (
    ...
    ...current,
    member_permissions: [
      ...current.member_permissions.filter(
        (row) =>
          !(
  ...
  const roleHas = (permission: PermissionKey, targetRole = role) =>
    data.role_permissions.some(
      (row) =>
        row.role === targetRole &&
        row.permission_key === permission &&
        row.access_channel === channel,
    );
  const memberOverride = (permission: PermissionKey) =>
    data.member_permissions.find(
      (row) =>
        row.member_id === memberId &&
        row.permission_key === permission &&
        row.access_channel === channel,
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Roles & permissions" size="4xl">
      <div className="space-y-5">
        <div className="seg-track flex w-full">
          <button
            type="button"
            className={`seg-item flex-1 ${targetType === 'role' ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className={`seg-item flex-1 ${targetType === 'member' ? 'is-active' : ''}`}
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex h-[38px] rounded-lg border border-white/[0.08] p-1">
            <button
              type="button"
              className={`h-full rounded-md px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-1 ${channel === 'app' ? 'bg-brand-600 text-white' : 'text-zinc-300'}`}
            ...
            <button
              type="button"
              className={`h-full rounded-md px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-1 ${channel === 'api' ? 'bg-brand-600 text-white' : 'text-zinc-300'}`}
        ...
        {targetType === 'role' ? (
          <p className="text-xs text-zinc-400">
            These defaults apply to everyone with the selected role. Owners always have full access.
          </p>
        ) : (
          <p className="text-xs text-zinc-400">
            Set exceptions for {selectedMember?.name || 'this member'}. A dashed box follows the{' '}
            {selectedMember?.role || 'role'} default (ghost check = on); solid teal/red is an
            explicit exception. Click a row to cycle Inherit → Allow → Deny.
          </p>
        )}
        {loading ? (
          <div className="py-10 text-center text-sm text-zinc-400">Loading access policy...</div>
        ) : (
          <div className="grid md:grid-cols-2 gap-4">
            {PERMISSION_GROUPS.map((group) => ({
              ...
              permissions: group.permissions.filter((permission) =>
                channel === 'api'
                  ? API_ENDPOINT_PERMISSION_SET.has(permission.key)
                  : ![
                    ...
                    {group.permissions.map((permission) => {
                      ...
                      const inherited =
                        isMember && selectedMember
                          ? roleHas(permission.key, selectedMember.role as TeamRole)
                          : false;
                      ...
                      const onRowClick = () => {
                        if (isMember) {
                          const next: PermissionEffect | null =
                            override === null ? 'allow' : override === 'allow' ? 'deny' : null;
                      ...
                      if (isMember) {
                        ...
                        tag = (
                          <span
                            className={`text-[11px] font-medium tabular-nums ${override === 'allow' ? 'text-brand-300' : override === 'deny' ? 'text-red-400' : 'text-zinc-500'}`}
                          >
                            {override === 'allow'
                              ? 'Allow'
                              : override === 'deny'
                      ...
                      const stateLabel = isMember
                        ? override === 'allow'
                          ...
                          : override === 'deny'
                            ? 'Denied'
                            : `inheriting the role default (${inherited ? 'on' : 'off'})`
                        : roleOn
                          ? 'enabled'
                          : 'disabled';
                      ...
                      return (
                        <button
                          ...
                          title={
                            isMember
                              ? `Inherit follows the ${selectedMember?.role || 'role'} default (currently ${inherited ? 'on' : 'off'}). Click to cycle: Inherit → Allow → Deny.`
                              : undefined
                          }
