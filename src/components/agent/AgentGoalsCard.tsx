...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  achieved: { label: 'Achieved', bg: 'bg-blue-500/15', text: 'text-blue-300' },
  paused: { label: 'Paused', bg: 'bg-amber-500/15', text: 'text-amber-300' },
  abandoned: { label: 'Abandoned', bg: 'bg-white/[0.06]', text: 'text-zinc-400' },
};
    ...
    <div className={`glass-card rounded-xl overflow-hidden flex flex-col ${showForm ? '' : 'max-h-[420px]'}`}>
      ...
      <div className="px-5 py-4 flex items-center justify-between flex-shrink-0 border-b border-white/[0.06]">
        ...
        {showForm ? (
          <button
            onClick={resetForm}
            className="p-1.5 text-zinc-500 hover:text-zinc-300 transition-colors rounded-md hover:bg-white/[0.06]"
          >
            <X size={16} />
          </button>
        ) : (
            ...
            <div className="p-5 space-y-3">
              {activeGoals.map((goal) => {
                ...
                return (
                  <div
                    key={goal.id}
                    className="group rounded-lg border border-white/[0.06] bg-white/[0.03] hover:border-white/[0.08] transition-colors"
                  >
                    <div className="p-3.5">
                        ...
                        <div className="flex items-center gap-0.5 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-within:opacity-100 transition-opacity flex-shrink-0">
                          <Tooltip content="Edit">
                            <button
                              ...
                              className="p-1.5 text-zinc-500 hover:text-brand-300 transition-colors rounded-md hover:bg-white/[0.06]"
                            >
                              <Pencil size={13} />
                            </button>
                          </Tooltip>
                          <Tooltip content="Archive">
                            <button
                              ...
                              className="p-1.5 text-zinc-500 hover:text-red-500 transition-colors rounded-md hover:bg-white/[0.06]"
                            >
                              <Trash2 size={13} />
                            </button>
                          </Tooltip>
