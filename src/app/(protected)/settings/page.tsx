...
const NOTIF_GROUPS: {
  ...
  adminOnly?: boolean;
  agentOnly?: boolean;
  items: { key: NotificationCategory; label: string; desc: string }[];
}[] = [
  {
    group: 'Tasks',
    items: [
      ...
      { key: 'task_deleted', label: 'Task deleted', desc: 'A task is deleted' },
      {
        ...
        desc: 'Task moved to In Progress, Done, etc.',
      },
      { key: 'task_assignments', label: 'Assignments', desc: 'Someone is assigned to a task' },
      {
        key: 'task_updates',
        label: 'General updates',
        desc: 'Priority, title, and other task changes',
      },
  ...
  },
  {
    ...
    items: [
      { key: 'project_created', label: 'Project created', desc: 'A new project is created' },
      {
        ...
        desc: 'A project is archived or deleted',
      },
      ...
      { key: 'project_contacts', label: 'Project contacts', desc: 'Contacts linked or removed' },
      {
        ...
        desc: 'Portal enabled, disabled, or configured',
      },
      {
        key: 'portal_updates',
        label: 'Portal updates',
        desc: 'Timeline updates posted, edited, or removed',
      },
      {
        key: 'time_entries',
        label: 'Time tracking',
        desc: 'Time entries logged, updated, or deleted',
      },
      {
        key: 'entity_files',
        label: 'File attachments',
        desc: 'Files attached or removed from entities',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      {
        key: 'lead_interactions',
        label: 'Interactions',
        desc: 'Calls, emails, meetings added or updated',
      },
  ...
  },
  {
    ...
    items: [
      {
        key: 'agent_suggestions' as NotificationCategory,
        label: 'Suggestions',
        desc: 'AI agent submits or resubmits suggestions',
      },
      {
        key: 'agent_goals' as NotificationCategory,
        label: 'Goals',
        desc: 'Agent goals created, updated, or archived',
      },
      {
        key: 'agent_autonomous' as NotificationCategory,
        label: 'Autonomous toggle',
        desc: 'Autonomous agents enabled or disabled on a project',
      },
      {
        key: 'agent_activity' as NotificationCategory,
        label: 'Activity',
        desc: 'General AI agent activity and status updates',
      },
    ],
  },
];
...
export default function SettingsPage() {
  ...
  const currentAvatarSrc =
    ...
    (currentMember.avatar.startsWith('http') || currentMember.avatar.startsWith('blob:'))
      ? currentMember.avatar
      : undefined);
  ...
  const handleToggleEmailNotif = (category: NotificationCategory) => {
    ...
    toast(
      'success',
      isCurrentlyEnabled ? 'Email notification disabled' : 'Email notification enabled',
    );
  };
    ...
    try {
      const res = await fetch('/api/team-members/update-email', {
        ...
        body: JSON.stringify({
          ...
          new_email: userEmail.trim().toLowerCase(),
        }),
      });
      ...
      <div className="p-4 lg:p-6 space-y-6 max-w-3xl mx-auto">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            <div
              className={`p-2 rounded-lg ${timezone === 'UTC' ? 'bg-amber-500/15' : 'bg-emerald-500/15'}`}
            >
              <Globe
                className={timezone === 'UTC' ? 'text-amber-400' : 'text-emerald-400'}
                size={20}
              />
            </div>
            <div className="flex-1">
              ...
              <p className="text-sm text-zinc-400">
                All times in the app will be shown in this timezone
              </p>
            </div>
          </div>
          ...
          {timezone === 'UTC' && (
            <div className="flex items-start gap-2.5 mb-4 px-3 py-2.5 bg-amber-500/15 border border-amber-500/30 rounded-lg">
              ...
              <p className="text-sm text-amber-300">
                Your timezone is set to <span className="font-semibold">UTC</span>. Times may appear
                incorrect. Select your local timezone below so all dates and times display
                accurately.
              </p>
            </div>
          )}
        ...
        <section className="glass-card rounded-xl p-4 lg:p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">
                Choose how the app looks on this account. Syncs across your devices.
              </p>
            </div>
          </div>
        ...
        <section className="glass-card rounded-xl p-4 lg:p-6">
          ...
          {/* Email master toggle */}
          <div
            className={`flex items-center justify-between p-3 mb-6 rounded-lg border ${hasSmtp === false ? 'bg-white/[0.03] border-white/[0.06] opacity-60' : 'bg-white/[0.03] border-white/[0.08]'}`}
          >
            ...
            {NOTIF_GROUPS.filter((g) => {
              ...
              <div key={group}>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wide w-11 text-center">
                      In-App
                    </span>
                    <span
                      className={`text-[10px] font-medium uppercase tracking-wide w-11 text-center ${emailEnabled && hasSmtp !== false ? 'text-zinc-500' : 'text-zinc-600'}`}
                    >
                      Email
                    </span>
                  </div>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                {!showKeyForm && !revealedKey && (
                  <Button size="sm" onClick={() => setShowKeyForm(true)} icon={<Plus size={14} />}>
              ...
              <div className="mb-6 p-4 bg-white/[0.03] border border-white/[0.08] rounded-lg space-y-3">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={generatingKey || !keyName.trim() || keyScopes.length === 0}
                  >
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium flex-shrink-0 bg-brand-500/15 text-brand-300">
                            {key.scopes?.length || 0} scopes
                          </span>
                        </div>
                        <code className="text-xs text-zinc-400 font-mono mt-0.5 block">
                          {key.key_prefix}...****
                        </code>
          ...
          <section className="glass-card rounded-xl p-4 lg:p-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">
                  Replaces live data with sample data. Changes won&apos;t be saved.
                </p>
              </div>
              <button
                type="button"
