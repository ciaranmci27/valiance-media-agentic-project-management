    ...
    return (
      <div className="animate-fadeIn min-h-screen flex items-center justify-center">
        <div className="text-center">
          <UserCircle className="mx-auto mb-3 text-zinc-500" size={40} />
          <h3 className="font-medium text-zinc-300 mb-1">Contact not found</h3>
          <button
  ...
  return (
    <div className="animate-fadeIn min-h-screen">
      ...
      <div className="p-4 lg:p-6 space-y-6">
        ...
        <div className="glass-card rounded-xl overflow-hidden flex flex-col max-h-[300px]">
            ...
            <div className="flex items-center gap-4 pb-5 border-b border-white/[0.06]">
              ...
              <div className="min-w-0">
                <div className="flex flex-col lg:flex-row lg:items-center lg:gap-5">
                  <h2 className="text-lg font-semibold text-white truncate">{contact.name}</h2>
                  {contact.company && (
                    <p className="text-sm text-zinc-400 mt-0.5 truncate lg:hidden">{contact.company}</p>
                  )}
                  <div className="flex items-center gap-4 text-sm mt-1 lg:mt-0">
                    ...
                    {contact.phone ? (
                      <a href={`tel:${contact.phone}`} className="flex items-center gap-1.5 text-zinc-300 hover:text-brand-300 transition-colors">
        ...
        <div className="glass-card rounded-xl overflow-hidden flex flex-col max-h-[300px]">
          ...
          {linkedLeads.length > 0 ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-xs text-zinc-500 mt-1">Leads linked to this contact will appear here</p>
            </div>
          )}
        </div>
        </div>
            ...
            <div className="glass-card rounded-xl overflow-hidden flex flex-col">
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.08] flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-white">Invoices ({contactInvoices.length})</h2>
                </div>
                <div className="flex items-center gap-4 text-xs">
                  <div>
                    <span className="text-zinc-500">Paid</span>
                    <span className="ml-1.5 font-semibold text-emerald-400">${totalPaid.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div>
                    <span className="text-zinc-500">Outstanding</span>
                    <span className="ml-1.5 font-semibold text-white">${totalOutstanding.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
              </div>
              {/* Invoice cards */}
              <div className="flex-1 overflow-y-auto p-4 space-y-2" style={{ maxHeight: '250px' }}>
                {contactInvoices.map((invoice) => {
                  ...
                  return (
                    <div key={invoice.id} className="p-3 rounded-lg border border-white/[0.08] hover:border-white/[0.12] transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full ${statusColors[invoice.status] || 'bg-white/[0.06] text-zinc-300'}`}>
                      ...
                      {project && (
                        <p className="mt-1.5 text-xs text-zinc-500">{project.name}</p>
                      )}
                    </div>
                  );
                })}
          ...
          <div className="glass-card rounded-xl overflow-hidden flex flex-col max-h-[600px]">
            ...
            {linkedProjects.length > 0 ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                ...
                <p className="text-sm font-medium text-zinc-400">No projects linked yet</p>
                <p className="text-xs text-zinc-500 mt-1">Create a project to link it to this contact</p>
              </div>
            )}
          </div>
